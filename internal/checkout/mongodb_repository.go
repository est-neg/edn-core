...
// NewMongoPlanRepository returns a PlanRepository backed by MongoDB.
func NewMongoPlanRepository(client *mongo.Client, cfg config.MongoConfig) PlanRepository {
	return &mongoPlans{coll: client.Database(cfg.Database).Collection(cfg.CollectionPlans)}
}
...
func (r *mongoPlans) Create(ctx context.Context, plan Plan) error {
	...
	if err != nil {
		if mongo.IsDuplicateKeyError(err) {
			return fmt.Errorf("create plan: %w", ErrDuplicateOrder)
		}
...
func (r *mongoPlans) FindByID(ctx context.Context, id string) (*Plan, error) {
	var p Plan
	err := r.coll.FindOne(ctx, bson.D{{Key: "id", Value: id}}).Decode(&p)
	if err != nil {
...
func (r *mongoPlans) FindBySlugAndCycle(ctx context.Context, slug, billingCycle string) (*Plan, error) {
	var p Plan
	filter := bson.D{
		{Key: "slug", Value: slug},
	...
	if err != nil {
		...
		return nil, fmt.Errorf("find plan by slug: %w", err)
	}
	return &p, nil
}

func (r *mongoPlans) ListActive(ctx context.Context) ([]Plan, error) {
	cursor, err := r.coll.Find(ctx, bson.D{{Key: "active", Value: true}})
	if err != nil {
	...
	if err := cursor.All(ctx, &plans); err != nil {
		return nil, fmt.Errorf("decode active plans: %w", err)
	}
	return plans, nil
}

func (r *mongoPlans) Deactivate(ctx context.Context, id string, updatedAt time.Time) error {
